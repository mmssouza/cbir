...
limits = [(0,170),(4,300),(-30000,-500),(500,30000),(4,300),(0,170),(0.1,0.5),(4,300),(0,170),(0.,1.),(0.,1.),(0.,1.)]
...
class sim_ann:
 ...
 def __init__(self,f,T0,alpha,P,L):
  seed()
  self.fc = f
  self.s = self.gera_individuo()
  self.T = T0
  self.fit = self.fc(self.s)
  self.hall_of_fame = []   
  self.P = int(P)
  self.L = int(L)
  self.alpha = alpha
 ...
 def gera_individuo(self):
   ...
   for lim in enumerate(limits):
    i,maxim,minim = lim[0],lim[1][1],lim[1][0]
    aux[i] = aux[i]*(maxim-minim) + minim
 ...
 def Perturba(self,x):
  for i in range(Dim):
   if scipy.rand() < 0.3:
    maxim,minim = limits[i][1],limits[i][0] 
    aux = x[i] + 0.05*(maxim-minim)*scipy.randn()
    if not minim <= aux <= maxim:
     x[i] = rand()*(maxim-minim) + minim
    else:
     x[i] = aux  
